...
Authors: David Mutchler, Dave Fisher, Matt Boutell, Amanda Stouder,
         their colleagues and Andrew Novotny.  April 2018.
...
def problem4b(list_of_tuples_of_strings):
    ...
    #                    AND HAS AN ELEGANT SOLUTION.  DO NOT GET STUCK ON IT!
    # -------------------------------------------------------------------------
    length_list = len(list_of_tuples_of_strings)

    for k in range(length_list):
        length_tuple = len(list_of_tuples_of_strings[k])
        for x in range(length_tuple):
            #probably should've named list_of_tuples_of_strings something so I don't have [k][x], but it works
            if is_prime(len(list_of_tuples_of_strings[k][x])):
                return True
